...
def drop_sand(cave, limit, col, row, sand_units)
  ...
  if cave[row + 1][col - 1].nil?
    ...
    # added = 0
    # (limit - row).times do |i|
    #   if cave[row + i + 1][col - i - 1].nil? && !cave[row + i + 2][row - i - 1].nil? && row + i + 2 < limit
    #     cave[row + i + 1][col - i - 1] = SAND
    #     added += 1
    #   elsif cave[row + i + 2][row - i - 1].nil? && row + i + 2 < limit && cave[row + i + 1][col - i - 1].nil?
    #     return drop_sand(cave, limit,col - i - 1,row + i + 1, sand_units + added)
    #   end
    ...
    #   # end
    # end
    # return drop_sand(cave, limit,col - 1,row + 1, sand_units + added)
